...
class GoogleAuthenticator
{
     ...
     * @return string
     */
    public function current(?string $secret = null, float|int|null $time = null): string
    {
        $secret = $secret ?? $this->defaultSecret ?? "";
        if (!is_int($time)) {
            $time = floor(time() / 30);
        ...
        return str_pad(strval($pinValue), 6, "0", STR_PAD_LEFT);
    }
     ...
     * @return mixed
     */
    private function hash2int(string $bytes, int $start): int
    {
